...
func validBusSnapshotSource(city string) *busSnapshotSource {
	...
	bodies := map[string][]byte{
		...
		"bus_operator|" + city:     []byte(`[{"OperatorID":"OP1","OperatorName":{"Zh_tw":"測試客運"},"AuthorityCode":"TPE"}]`),
	...
	return &busSnapshotSource{bodies: bodies, times: times, errors: map[string]error{}}
}
...
func TestReadBusCitySnapshotPreservesIntentionalScheduleDuplicates(t *testing.T) {
	src := validBusSnapshotSource("Taipei")
	src.bodies["bus_schedule|Taipei"] = []byte(`[{"SubRouteUID":"TPE100","Direction":0,"Timetables":[{"TripID":"CIRCULAR","ServiceDay":{"Monday":1},"StopTimes":[
...
func TestReadBusCitySnapshotDeduplicatesIdenticalStopsAndRejectsDivergence(t *testing.T) {
	...
	src.bodies["bus_stopofroute|Taipei"] = []byte(`[
		{"SubRouteUID":"TPE100","Direction":0,"Stops":[{"StopUID":"S1","StopName":{"Zh_tw":"甲"},"StopSequence":1,"StationID":"ST1","StopPosition":{"PositionLon":121.5,"PositionLat":25}}]},
		{"SubRouteUID":"TPE100","Direction":0,"Stops":[{"StopUID":"S2","StopName":{"Zh_tw":"乙"},"StopSequence":1,"StationID":"ST2","StopPosition":{"PositionLon":121.6,"PositionLat":25}}]}
	]`)
...
func TestReadBusCitySnapshotMapsNativeFareAndRejectsDivergentCanonicalOffers(t *testing.T) {
	...
	src.bodies["bus_stopofroute|"+city] = []byte(`[{"SubRouteUID":"THB096801","Direction":0,"Stops":[{"StopUID":"S1","StopName":{"Zh_tw":"甲"},"StopSequence":1,"StationID":"ST1","StopPosition":{"PositionLon":121.5,"PositionLat":25}}]}]`)
...
func TestBusScheduleRowsUseDepartureTime(t *testing.T) {
	src := validBusSnapshotSource("Taipei")
	src.bodies["bus_schedule|Taipei"] = []byte(`[{"SubRouteUID":"TPE100","Direction":0,"Timetables":[{"TripID":"TRIP","ServiceDay":{"Monday":1},"StopTimes":[{"StopSequence":1,"StopUID":"S1","StopName":{"Zh_tw":"甲"},"ArrivalTime":"08:00","DepartureTime":"08:03"}]}]}]`)
	snapshot, err := readBusCitySnapshot(context.Background(), src, "Taipei")
	if err != nil {
		t.Fatalf("read snapshot: %v", err)
	}
...
func TestWriteBusCitySnapshotReturnsEveryStageFailure(t *testing.T) {
	for _, temp := range []string{
		"temp_bus", "temp_bus_stations", "temp_bus_groups", "temp_bus_members",
	...
	for _, fragment := range []string{
		...
		"INSERT INTO bus_subroutes", "INSERT INTO bus_stations", "INSERT INTO bus_station_groups",
		...
		"DELETE FROM bus_stations", "DELETE FROM bus_subroutes", "DELETE FROM bus_static",
	} {
