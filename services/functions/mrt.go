...
// system via a temp-table COPY and upsert, then prunes rows older than this
// load's start so retired schedule entries drop out. It consumes an
// already-opened decoder; the temp_mrt COPY, upsert, and post-commit prune
// (DELETE FROM mrt_schedule WHERE system=$1 AND updated_at<$2) are byte-identical
// to the legacy transform.
...
// mrt_journey_matrix from a decoder over the reconstructed raw_tdx array. Unlike
// the other transforms the legacy mrtJourneyMatrix consumed a resty result, not
// a decoder; this loader decodes []mrtODFare from the decoder instead, but the
// INSERT ... ON CONFLICT (from_station_id,to_station_id,system) batch is
// byte-identical to the legacy upsert (only the input source changed).
