...
func TestWaitForPipelineMarkerReturnsImmediatelyWhenReady(t *testing.T) {
	...
	err := waitForPipelineMarker(context.Background(), reader, "load", runDate,
		time.Minute, time.Hour, clock.now, noopSleep)
	if err != nil {
		t.Fatalf("waitForPipelineMarker() error = %v, want nil", err)
	}
	if reader.calls != 1 {
...
func TestWaitForPipelineMarkerPollsUntilReady(t *testing.T) {
	...
	err := waitForPipelineMarker(context.Background(), reader, "changetovector",
...
func TestWaitForPipelineMarkerGivesUpAtDeadline(t *testing.T) {
	...
	err := waitForPipelineMarker(context.Background(), reader, "load", time.Now(),
		5*time.Minute, 2*time.Hour, clock.now, noopSleep)
	if err == nil {
		t.Fatal("waitForPipelineMarker() error = nil, want deadline error")
	}
	if reader.calls != 3 {
...
func TestWaitForPipelineMarkerKeepsPollingOnReaderError(t *testing.T) {
	...
	if !errors.Is(err, readErr) {
		t.Fatalf("waitForPipelineMarker() error = %v, want deadline error wrapping %v", err, readErr)
	}
	if reader.calls < 2 {
...
func TestWaitForPipelineMarkerRecoversFromTransientReaderError(t *testing.T) {
	...
	err := waitForPipelineMarker(context.Background(), reader, "load", time.Now(),
		5*time.Minute, 2*time.Hour, clock.now, noopSleep)
	if err != nil {
		t.Fatalf("waitForPipelineMarker() error = %v, want nil after transient read error", err)
	}
	if reader.calls != 2 {
...
func TestWaitForPipelineMarkerPropagatesSleepError(t *testing.T) {
	...
	if !errors.Is(err, wantErr) {
		t.Fatalf("waitForPipelineMarker() error = %v, want %v", err, wantErr)
	}
}
