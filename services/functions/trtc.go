...
// writes the same Redis keys/channels mrtEta wrote so the router and app are
...
func trtcPublish(ctx context.Context, sink liveSink, names map[string][]string, windows map[string][]mrtServiceWindow, now time.Time, tracks []trtcTrack, exRows []trtcWeightEx, brRows []trtcWeightBR) error {
