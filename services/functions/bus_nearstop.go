...
func nearStopRecords(ctx context.Context, fetch boundFetch, city string) ([]rawBusNearStop, error) {
	...
	if !result.Modified {
		return nil, invalidateTDXFetch(result)
	}
	rows := make([]rawBusNearStop, 0)
	if err := commitTDXFetch(result, func(dec *json.Decoder) error {
		return decodeLiveItems(dec, func(r rawBusNearStop) error {
			rows = append(rows, r)
			return nil
		})
	}); err != nil {
...
// _busStopEventCols is the archive table's column order; see
