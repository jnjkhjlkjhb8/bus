...
func TestReminderClaimTimeoutExceedsInFlightSendBound(t *testing.T) {
	if liveJobTimeout+notify.ArrivalFinalizationTimeout >= notify.ReminderClaimTimeout {
		t.Fatalf(
			...
			liveJobTimeout, notify.ArrivalFinalizationTimeout, notify.ReminderClaimTimeout,
		)
	}
}
