...
func TestBuildNearStopIndex(t *testing.T) {
	now := time.Date(2026, 8, 9, 8, 0, 0, 0, _taipei)
...
func TestBusStopEventRows(t *testing.T) {
	...
	want := []any{
		...
		int16(1), event, time.Date(2026, 8, 9, 7, 30, 0, 0, _taipei), int16(1), now,
	}
	if len(rows[0]) != len(_busStopEventCols) {
		t.Fatalf("row width = %d, want %d columns", len(rows[0]), len(_busStopEventCols))
	}
	for i, got := range rows[0] {
		if gotTime, ok := got.(time.Time); ok {
			if !gotTime.Equal(want[i].(time.Time)) {
				t.Errorf("column %s = %v, want %v", _busStopEventCols[i], gotTime, want[i])
			}
			continue
		}
		if got != want[i] {
			t.Errorf("column %s = %v, want %v", _busStopEventCols[i], got, want[i])
		}
	}
