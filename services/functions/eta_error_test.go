...
func TestMatchPredictionActual(t *testing.T) {
	...
	preds := []predictionRecord{
		...
		{subRouteUID: "R1", direction: 0, stopUID: "S1", source: _sourceModel, predictedAt: base, predictedSecs: 240},
		// No arrival at this stop => dropped.
		{subRouteUID: "R1", direction: 0, stopUID: "S9", source: _sourceTDX, predictedAt: base, predictedSecs: 100},
		// Arrival exists but before predictedAt => dropped.
		{subRouteUID: "R2", direction: 1, stopUID: "S2", source: _sourcePropagation, predictedAt: base.Add(20 * time.Minute), predictedSecs: 60},
		// Arrival exists but beyond the window => dropped.
		{subRouteUID: "R3", direction: 0, stopUID: "S3", source: _sourceSchedule, predictedAt: base, predictedSecs: 60},
	}
	...
	m := got[0]
	if m.subRouteUID != "R1" || m.stopUID != "S1" || m.source != _sourceModel {
...
func TestMatchPredictionActual_PicksEarliestArrival(t *testing.T) {
	base := time.Date(2026, 7, 6, 8, 0, 0, 0, time.UTC)
	preds := []predictionRecord{
		{subRouteUID: "R1", direction: 0, stopUID: "S1", source: _sourceTDX, predictedAt: base, predictedSecs: 120},
	}
...
func TestAggregateMAE(t *testing.T) {
	errs := []matchedError{
		...
		{subRouteUID: "R1", source: _sourceModel, predictedSecs: 100, actualSecs: 120},
		{subRouteUID: "R1", source: _sourceModel, predictedSecs: 200, actualSecs: 160},
		// R1/propagation: |50-60|=10 => MAE 10 over 1.
		{subRouteUID: "R1", source: _sourcePropagation, predictedSecs: 50, actualSecs: 60},
		// R2/tdx: |300-300|=0 => MAE 0 over 1.
		{subRouteUID: "R2", source: _sourceTDX, predictedSecs: 300, actualSecs: 300},
	}
	...
	want := []maeStat{
		{subRouteUID: "R1", source: _sourceModel, maeSeconds: 30, samples: 2},
		{subRouteUID: "R1", source: _sourcePropagation, maeSeconds: 10, samples: 1},
		{subRouteUID: "R2", source: _sourceTDX, maeSeconds: 0, samples: 1},
	}
	for i, w := range want {
		g := got[i]
