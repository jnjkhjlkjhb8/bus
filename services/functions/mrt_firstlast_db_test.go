...
//
// Drives loadMrtFirstlast directly through pgLoadSink because runLoad only
...
func TestLoadMrtFirstlastCollapsesDuplicateNaturalKeys(t *testing.T) {
	...
	if err := loadMrtFirstlast(ctx, decodeInto(body), sink, system); err != nil {
		t.Fatalf("loadMrtFirstlast tripped the unique constraint (this is the bug): %v", err)
	}
