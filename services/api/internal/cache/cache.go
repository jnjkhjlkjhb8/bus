// Package cache is the router's in-process TTL cache: a bounded map that memoizes
// static PostgreSQL reads (route payloads, station groups, the static-version
// stamp) so a hot route is answered without a round trip. Entries expire by
// wall clock; nothing here is shared with Redis.
package cache
