// Package gtfs builds the transit feeds MOTIS plans on: the nightly static GTFS
// export from the landed schema, and the GTFS-Realtime snapshot rebuilt on a
// short cadence from live bus positions and ETAs. Bus schedules are
// origin-only in the source data, so stop times are accumulated from measured
// segment travel times rather than published per stop.
package gtfs
...
// RunExport builds one feed and publishes it, logging rather than returning
